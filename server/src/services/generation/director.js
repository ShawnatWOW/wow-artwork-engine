...
HARD RULES: name only characters visibly present in the image. EVERY character moves in EVERY moment — nothing ever stands still, poses, or watches idly. The ONE inviolable boundary is the picture's outer edge: even at its closest and largest, every character keeps its ENTIRE silhouette inside the image — never clipped by the image's edge, never filling the whole screen (in front of the frame: yes; past the picture's edge: never). FORBIDDEN words and devices: timestamps, second counts, numbered beats or parts, and the words "cut", "shot", "scene two", "act", "camera", "transition", "frame two".
